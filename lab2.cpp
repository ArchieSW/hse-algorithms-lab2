...
class BruteForceSolution {
 ...
 private:
  Nonnull<const Rectangles*> rectangles;
};
...
class MapSolution {
 public:
  ...
  void prepare_rectangles(Rectangles& recs) {
    rectangles = &recs;
    compress_rectangles();
    fill_map();
  }
  ...
  auto get_number_of_crossing(const Vec2d& query) -> int {
    static bool is_outed = false;
    if (!is_outed) {
      for (const auto& row : solution_map) {
    ...
    if (compressed.x >= static_cast<int>(compressed_x.size()) ||
        compressed.y >= static_cast<int>(compressed_y.size())) {
      return 0;
    }
...
// static_assert(Solution<PersistentSegmentTreeSolution>, "PersistentSegmentTreeSolution is not implement needed methods");

template <Solution T>
void solve_with_solution(T& solution, Rectangles& recs, const Points& queries) {
  solution.prepare_rectangles(recs);
  for (auto& query : queries) {
    std::cout << solution.get_number_of_crossing(query) << ' ';
  }
}
...
auto main() -> int {
  ...
  PersistentSegmentTreeSolution map_sol;

  solve_with_solution(map_sol, recs, queries);
  return 0;
}
